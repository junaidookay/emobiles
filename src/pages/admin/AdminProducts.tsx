                ...
                <tbody>
                  {filtered.map(product => (
                    <tr key={product.id} className="border-b last:border-0 hover:bg-secondary/30">
                      ...
                      <td className="py-3 px-4 font-medium">PKR {product.discount_price ?? product.price}</td>
                      <td className="py-3 px-4">
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div><Label>Base Price (PKR)</Label><Input className="mt-1" type="number" value={form.price} onChange={e => setForm(p => ({ ...p, price: e.target.value }))} /></div>
